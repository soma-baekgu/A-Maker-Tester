...
export function setup() {
    ...
    /* Parallel user login */users.push(...Array(options.vus).fill().map((_, i) => {
        const loginResponse = http.post(`${BASE_URL}/api/v1/auth/code/google?code=${randomString(8)}`);
        const user = JSON.parse(loginResponse.body).data;
        return user;
    }));
    ...
    // Parallel workspace creation and activation
    users.forEach(user => {
        const createWorkspaceResponse = http.post(`${BASE_URL}/api/v1/workspaces`, JSON.stringify({
            name: "워크스페이스",
        ...
        // Activate workspace invitation for each user
        const headers = {
            'Authorization': `Bearer ${user.token}`,
            'Content-Type': 'application/json',
        };
        const endpoint = `${BASE_URL}/api/v1/workspaces/${userWorkspaces[userWorkspaces.length - 1].workspaceId}/invite/activate`;
    ...
    const first = chatRooms[chatRooms.length - 1].chatRoomId;
